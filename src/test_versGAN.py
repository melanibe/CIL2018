...
# Load data
#data_query = preprocessing.load_data("query", path = cwd+"/produced/1530011289/")
	...
	with sess.as_default():
		...
		with open("{}.csv".format(out_file),"w") as file:
			...
			for test_batch in batches: # 
				...
				#print(tf.losses.absolute_difference(labels=batch_score, predictions = batch_scores, reduction=tf.losses.Reduction.MEAN))
				#print(np.mean(np.abs(batch_scores - batch_score)))
