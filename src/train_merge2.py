...
from model.model_merge2 import Discriminator
from config import *
import pandas as pd
...
#dev_sample_index = int(FLAGS.dev_sample_percentage * float(len(shuffled_indices)))
test_indices = shuffled_indices[:dev_sample_index] #to keep the same size for eval
train_indices = shuffled_indices[dev_sample_index:]
...
with graph.as_default():
	...
	with sess.as_default():
		...
		for batch_label, batch_score in zip(batches_label, batches_score):
			...
			print(np.shape(batch_imgs_label))
