...
class Discriminator(object):
	...
	def __init__(self, reuse=False, discr_type="regressor", filter_height = 5, filter_width = 5, out_channels1 = 8, out_channels2 = 16):
		...
		self.input_img_score = tf.placeholder(tf.float32, [None, 1000, 1000], name='input_img')  # dim batch * shape
		self.input_img_label = tf.placeholder(tf.float32, [None, 1000, 1000], name='input_img')  # dim batch * shape
		...
		with tf.device('/gpu:0'):
			...
			with tf.name_scope("output"):
				...
				predictions_score = tf.reshape(tf.minimum(8.0, tf.layers.dense(pool2_flat_scores, units=1, activation=tf.nn.relu)), [-1], name="score_pred")
				
				logits = tf.layers.dense(pool2_flat_labels, units=2, activation=tf.nn.relu, name="logits")
	...
	def CNN(self, input, out_channels1,filter_height, filter_width, out_channels2):
		...
		with tf.variable_scope("conv1"):
			h_conv1 = tf.layers.conv2d(inputs= input, \
