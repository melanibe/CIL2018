...
This file is a helper file to build the label discriminant.
"""
...
def discriminator_label(input_images, reuse=False, out_channels1=32, out_channels2=8, filter_height=3, filter_width=3):
	"""Fonction building the label discriminator. 
	Args:
	...
	Returns: 
		logits(None, 2): the logits associated to each image in the input. Used during training 
						 for cross_entropy loss calculation.
	"""
	with tf.variable_scope("discr_label") as scope:
		if (reuse):
			tf.get_variable_scope().reuse_variables()
		# Reshaping the input (necessary for convoluational layer input)
		input = tf.expand_dims(input_images, 3)
		with tf.name_scope("CNN"):
            # First pooling layers to reduce the dim of the images
			...
			with tf.name_scope("third_conv_pool"):
				...
				pool3_flat = tf.reshape(h_pool3, [-1, shape[1]*shape[2]*shape[3]])
				print(pool3_flat.get_shape())
		with tf.name_scope("fully_connected"):
			h_fc1 = tf.layers.dense(pool3_flat, units=28, activation=tf.nn.relu)
		with tf.name_scope("output"):
			logits = tf.layers.dense(h_fc1, units=2, activation=tf.nn.relu, name="logits")
		return logits
